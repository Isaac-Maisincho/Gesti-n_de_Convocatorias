...
type Convocatoria = { id: string; anio: number; estado: string };

type Director = { id: string; nombre: string; correo: string; telefono: string };

type DepartamentoCarrera = {
  id: string;
  nombre: string;
...
type NotasRes = ApiResponse<{
  datos: NotaResumen[];
  paginacion: { totalRegistros: number };
}>;

type NotaDetalle = ApiResponse<{
  codigo: string;
  nombreProyecto: string;
  estado: string;
  ...
  departamento: string;
  directorId: string;
  localizacion: { cobertura: string; provincia?: string; canton?: string; parroquia?: string; detalleUbicacion: string };
  ...
  impactosEsperados: { economico: string; social: string; politico: string; cientifico: string; ambiental: string; otros: string };
  alineamiento: { [key: string]: string[] };
  departamentosParticipantes: string[];
  carrerasParticipantes: string[];
  firmasResponsabilidad: { directorNota: string; directorCarrera: string; coordinadorVinculacion: string; directorDepartamento: string };
}>;

type EstadisticasRes = ApiResponse<{ presupuestoTotal: number; promedioPorNota: number; conteosPorEstado: Record<string, number> }>;

type UbicacionProvincia = {
  id: string;
  nombre: string;
};
...
type ResumenNotas = ApiResponse<{ datos: NotaResumen[]; paginacion: { totalRegistros: number } }>;
...
const cronogramaPanel = document.getElementById('cronograma-panel') as HTMLElement;
const firmasPanel = document.getElementById('firmas-panel') as HTMLElement;
const searchCodeInput = document.getElementById('search-code') as HTMLInputElement | null;
const searchButton = document.getElementById('search-button') as HTMLButtonElement | null;
const openNoteFormButton = document.getElementById('open-note-form') as HTMLButtonElement | null;
const newNoteButton = document.getElementById('new-note') as HTMLButtonElement | null;

let catalogosData: CatalogosData | null = null;
let departamentosData: DepartamentoCarrera[] = [];
let provinciasData: UbicacionProvincia[] = [];
...
const ALINEAMIENTO_GRUPOS: Array<{ key: keyof CatalogosData; label: string }> = [
...
async function fetchJson<T>(path: string, options?: RequestInit): Promise<T | null> {
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error((errorData as any).mensaje || response.statusText);
    }
...
async function loadInitialData(): Promise<void> {
  ...
  const [catalogosRes, departamentosRes, ubicacionesRes] = await Promise.all([
    fetchJson<ApiResponse<CatalogosData>>('/api/catalogos'),
    fetchJson<ApiResponse<DepartamentoCarrera[]>>('/api/departamentos-carreras'),
    fetchJson<ApiResponse<UbicacionProvincia[]>>('/api/ubicaciones'),
  ]);
...
function buildList(container: HTMLElement, items: Array<{ title: string; subtitle: string }>) {
...
async function refreshConvocatorias() {
  const conv = await fetchJson<ApiResponse<Convocatoria[]>>('/api/convocatorias');
...
async function refreshDirectores() {
  const data = await fetchJson<ApiResponse<Director[]>>('/api/directores');
...
async function viewNoteDetail(codigo: string) {
  ...
  const closeButton = modal?.querySelector('[data-close="modal"]') as HTMLButtonElement | null;
  ...
  document.getElementById('update-status-button')?.addEventListener('click', async () => {
    ...
    const result = await patchJson<ApiResponse<{ estadoAnterior: string; estadoNuevo: string }>>(`/api/notas/${codigo}/estado`, { estado });
...
async function refreshCronograma() {
  const notasSummary = await fetchJson<NotasRes>('/api/notas?limite=100');
  ...
  const detallesResponses = await Promise.all(
    notasSummary.datos.datos.map((nota) => fetchJson<NotaDetalle>(`/api/notas/${nota.codigo}`))
  );

  const actividades = detallesResponses
    .filter((response): response is NotaDetalle => !!response && response.exito && !!response.datos)
  ...
  cronogramaPanel.querySelectorAll('[data-update]').forEach((button) => {
    button.addEventListener('click', async () => {
      const [codigo, actividadId] = (button as HTMLElement).dataset.update!.split('|');
...
async function editActividadCronograma(codigo: string, actividadId: string) {
  const notaDetalle = await fetchJson<NotaDetalle>(`/api/notas/${codigo}`);
  ...
  document.getElementById('save-actividad-button')?.addEventListener('click', async () => {
    const descripcion = (document.getElementById('edit-actividad-descripcion') as HTMLInputElement).value.trim();
    const fechaInicio = (document.getElementById('edit-actividad-inicio') as HTMLInputElement).value;
    const fechaFin = (document.getElementById('edit-actividad-fin') as HTMLInputElement).value;
    const responsable = (document.getElementById('edit-actividad-responsable') as HTMLInputElement).value.trim();
    ...
    const nuevasActividades = notaDetalle.datos!.cronograma.map((item) =>
      item.id === actividadId ? { ...item, descripcion, fechaInicio, fechaFin, responsable } : item
    );

    const result = await patchJson<ApiResponse<NotaDetalle>>(`/api/notas/${codigo}`, { cronograma: nuevasActividades });
    if (result && result.exito) {
...
async function openNoteForm() {
  ...
  const convocatoriaSelect = document.getElementById('nota-convocatoria') as HTMLSelectElement;
  const directorSelect = document.getElementById('nota-director') as HTMLSelectElement;
  const provinciaSelect = document.getElementById('ubicacion-provincia') as HTMLSelectElement;
  const cantonSelect = document.getElementById('ubicacion-canton') as HTMLSelectElement;
  const parroquiaSelect = document.getElementById('ubicacion-parroquia') as HTMLSelectElement;
  const alignmentContainer = document.getElementById('alignment-container') as HTMLElement;
  const addBudgetRowButton = document.getElementById('add-budget-row') as HTMLButtonElement;
  const addScheduleRowButton = document.getElementById('add-schedule-row') as HTMLButtonElement;
  const saveNoteButton = document.getElementById('save-note-button') as HTMLButtonElement;
  const alignmentPrevButton = document.getElementById('alignment-prev') as HTMLButtonElement;
  const alignmentNextButton = document.getElementById('alignment-next') as HTMLButtonElement;

  const directoresRes = await fetchJson<ApiResponse<Director[]>>('/api/directores');
  const convocatoriasRes = await fetchJson<ApiResponse<Convocatoria[]>>('/api/convocatorias');
  ...
  provinciaSelect?.addEventListener('change', async () => {
    ...
    const provinciaDetalle = await fetchJson<ApiResponse<{ provincia: string; cantones: { id: string; nombre: string }[] }>>(
      `/api/ubicaciones?provinciaId=${encodeURIComponent(provinciaId)}`
    );
  ...
  cantonSelect?.addEventListener('change', async () => {
    ...
    const parroquiasRes = await fetchJson<ApiResponse<CantonResponse>>(
      `/api/ubicaciones?provinciaId=${encodeURIComponent(provinciaId)}&cantonId=${encodeURIComponent(cantonId)}`
    );
  ...
  function addBudgetRow() {
    ...
    const quantityInput = wrapper.querySelector('.budget-quantity') as HTMLInputElement;
    const priceInput = wrapper.querySelector('.budget-price') as HTMLInputElement;
    const totalInput = wrapper.querySelector('.budget-total') as HTMLInputElement;
    const removeButton = wrapper.querySelector('.remove-row') as HTMLButtonElement;
  ...
  saveNoteButton?.addEventListener('click', async () => {
    const nombreProyecto = (document.getElementById('nota-nombre') as HTMLInputElement).value.trim();
    const convocatoriaId = (document.getElementById('nota-convocatoria') as HTMLSelectElement).value;
    const sedeUnidadAcademica = (document.getElementById('nota-sede') as HTMLSelectElement).value;
    const departamentoSeleccionado = (document.getElementById('nota-departamento') as HTMLSelectElement).value;
    const directorId = (document.getElementById('nota-director') as HTMLSelectElement).value;
    const fechaInicio = (document.getElementById('nota-fecha-inicio') as HTMLInputElement).value;
    const fechaFin = (document.getElementById('nota-fecha-fin') as HTMLInputElement).value;
    const detalleUbicacion = (document.getElementById('ubicacion-detalle') as HTMLTextAreaElement).value.trim();
    const provincia = (document.getElementById('ubicacion-provincia') as HTMLSelectElement).value;
    const canton = (document.getElementById('ubicacion-canton') as HTMLSelectElement).value;
    const parroquia = (document.getElementById('ubicacion-parroquia') as HTMLSelectElement).value;
    const urbanoMarginal = (document.getElementById('poblacion-urbanoMarginal') as HTMLInputElement).checked;
    const rural = (document.getElementById('poblacion-rural') as HTMLInputElement).checked;
    const grupoAtencionPrioritaria = (document.getElementById('poblacion-grupoAtencionPrioritaria') as HTMLInputElement).checked;
    const impactoEconomico = (document.getElementById('impacto-economico') as HTMLTextAreaElement).value.trim();
    const impactoSocial = (document.getElementById('impacto-social') as HTMLTextAreaElement).value.trim();
    const impactoPolitico = (document.getElementById('impacto-politico') as HTMLTextAreaElement).value.trim();
    const impactoCientifico = (document.getElementById('impacto-cientifico') as HTMLTextAreaElement).value.trim();
    const impactoAmbiental = (document.getElementById('impacto-ambiental') as HTMLTextAreaElement).value.trim();
    const impactoOtros = (document.getElementById('impacto-otros') as HTMLTextAreaElement).value.trim();
    const poblacionReferencia = Number((document.getElementById('poblacion-referencia') as HTMLInputElement).value);
    const poblacionPotencial = Number((document.getElementById('poblacion-potencial') as HTMLInputElement).value);
    const poblacionObjetivo = Number((document.getElementById('poblacion-objetivo') as HTMLInputElement).value);
    const firmaDirectorNota = (document.getElementById('firma-directorNota') as HTMLInputElement).value.trim();
    const firmaDirectorCarrera = (document.getElementById('firma-directorCarrera') as HTMLInputElement).value.trim();
    const firmaCoordinadorVinculacion = (document.getElementById('firma-coordinadorVinculacion') as HTMLInputElement).value.trim();
    const firmaDirectorDepartamento = (document.getElementById('firma-directorDepartamento') as HTMLInputElement).value.trim();

    const departamentosParticipantes = Array.from(document.querySelectorAll<HTMLInputElement>('input[name="departamento"]:checked')).map((input) => input.value);
    const carrerasParticipantes = Array.from(document.querySelectorAll<HTMLInputElement>('input[name="carrera"]:checked')).map((input) => input.value);

    const alineamientoSeleccionado: Record<string, string[]> = {};
    ALINEAMIENTO_GRUPOS.forEach((group) => {
      const valores = Array.from(document.querySelectorAll<HTMLInputElement>(`input[name="alineamiento-${group.key}"]:checked`)).map((input) => input.value);
      alineamientoSeleccionado[group.key] = valores;
    });

    const presupuestoItems = Array.from(document.querySelectorAll<HTMLDivElement>('.budget-row')).map((row) => {
      const descripcion = (row.querySelector('.budget-description') as HTMLInputElement).value.trim();
      const cantidad = Number((row.querySelector('.budget-quantity') as HTMLInputElement).value);
      const valorUnitario = Number((row.querySelector('.budget-price') as HTMLInputElement).value);
      const total = Math.round(cantidad * valorUnitario * 100) / 100;
      return { id: createId(), descripcion, cantidad, valorUnitario, total };
    });

    const cronograma = Array.from(document.querySelectorAll<HTMLDivElement>('.schedule-row')).map((row) => {
      const descripcion = (row.querySelector('.schedule-description') as HTMLInputElement).value.trim();
      const fechaInicio = (row.querySelector('.schedule-start') as HTMLInputElement).value;
      const fechaFin = (row.querySelector('.schedule-end') as HTMLInputElement).value;
      const responsable = (row.querySelector('.schedule-responsable') as HTMLInputElement).value.trim();
    ...
    const result = await postJson<ApiResponse<NotaDetalle>>('/api/notas', notaPayload);
    if (!result) return;
    if (!result.exito) {
...
const saveConvocatoriaButton = document.getElementById('save-convocatoria') as HTMLButtonElement | null;
const saveDirectorButton = document.getElementById('save-director') as HTMLButtonElement | null;

async function saveConvocatoria() {
  const yearInput = document.getElementById('convocatoria-year') as HTMLInputElement;
  const statusSelect = document.getElementById('convocatoria-status') as HTMLSelectElement;
  ...
  const result = await postJson<ApiResponse<Convocatoria>>('/api/convocatorias', { anio, estado });
  if (!result) return;
  if (!result.exito) {
...
async function saveDirector() {
  const nameInput = document.getElementById('director-name') as HTMLInputElement;
  const emailInput = document.getElementById('director-email') as HTMLInputElement;
  const phoneInput = document.getElementById('director-phone') as HTMLInputElement;
  ...
  const result = await postJson<ApiResponse<Director>>('/api/directores', { nombre, correo, telefono });
  if (!result) return;
  if (!result.exito) {
    showNotification(result.mensaje, 'error');
    return;
